...
(function () {
  ...
  /* 着用イメージボタンの表示制御：
   *  - モデル着用写真（wornPhotos）がある商品 → 表示（実写の着用イメージ）
   *  - 実写商品だが着用写真が無い → 非表示（旧トルソー演出は実写と混ざり破綻するため）
   *  - イラスト商品 → 従来のトルソー演出を表示 */
  function updateWornButton() {
    const btn = $("#btnWorn");
    if (!btn) return;
    const p = Editor.state.product;
    const hasWornPhotos = !!(p && p.wornPhotos);
    const photoBacked = !!(p && Mockups.photoFor && Mockups.photoFor(p, Editor.state.colorId, "front"));
    const show = !!p && (hasWornPhotos || (Mockups.isWearable(p) && !photoBacked));
    btn.hidden = !show;
